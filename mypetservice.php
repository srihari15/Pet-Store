...
$first_name = mysqli_real_escape_string($link, $_REQUEST['fname']);
 if (empty($first_name)) {
    ...
    window.location='service.html'</script>";
  } else {
...
$sql =  $link->query("INSERT INTO user (email,password,roleid) VALUES ('$email',1234567,1)");
if($sql){
$sql2 = $link->query("Select userid from user where email='$email'");
$row= mysqli_fetch_assoc($sql2);
$userid = $row['userid'];
$sql1 = $link->query("INSERT INTO service (fname,lname,email,phone,userid) VALUES ('$first_name','$last_name','$email',100,'$userid')");
}
if($sql1){
